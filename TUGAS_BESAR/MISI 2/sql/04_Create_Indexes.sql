...
IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'ix_fact_surat_in_process' AND object_id = OBJECT_ID('fact.fact_surat'))
    CREATE INDEX ix_fact_surat_in_process ON fact.fact_surat(tanggal_key, status_akhir) WHERE status_akhir <> 'Selesai' AND status_akhir <> 'Arsip';
...
PRINT '>> All indexes created for Dimensions, Facts, and ETL logs.';
GO
