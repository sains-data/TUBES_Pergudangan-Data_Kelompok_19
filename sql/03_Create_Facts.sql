...
CREATE INDEX IF NOT EXISTS ix_fact_surat_tgl ON fact.fact_surat(tanggal_key);
CREATE INDEX IF NOT EXISTS ix_fact_surat_jenis ON fact.fact_surat(jenis_surat_key);
CREATE INDEX IF NOT EXISTS ix_fact_surat_unit ON fact.fact_surat(unit_pengirim_key);
...
CREATE TABLE IF NOT EXISTS fact.fact_layanan (
    ...
    -- Measures
    sla_target_jam INT,
    waktu_respon_jam NUMERIC(10,2),
    waktu_selesai_jam NUMERIC(10,2),
    rating_kepuasan NUMERIC(3,2),
...
CREATE INDEX IF NOT EXISTS ix_fact_layanan_tgl ON fact.fact_layanan(tanggal_request_key);
CREATE INDEX IF NOT EXISTS ix_fact_layanan_jenis ON fact.fact_layanan(jenis_layanan_key);
...
CREATE TABLE IF NOT EXISTS fact.fact_aset (
    ...
    umur_ekonomis_tahun NUMERIC(5,2),
    umur_tersisa_tahun NUMERIC(5,2),
...
CREATE INDEX IF NOT EXISTS ix_fact_aset_tgl ON fact.fact_aset(tanggal_snapshot_key);
CREATE INDEX IF NOT EXISTS ix_fact_aset_barang ON fact.fact_aset(barang_key);
CREATE INDEX IF NOT EXISTS ix_fact_aset_unit ON fact.fact_aset(unit_pemilik_key);

-- =====================================================
-- SUCCESS NOTICE
-- =====================================================

SELECT 'Fact tables and indexes created successfully in schema "fact".' as status;
